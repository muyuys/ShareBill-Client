...
Page({
  ...
  async handleFormSubmit() {
    ...
    let reason = this.data.reason[this.data.reasonIndex];
    let order_id = this.data.order_id;
    // 上传举报文本其他信息
    const res = await request({
      url: "/report",
      data: {
        introduction: textVal,
        openid,
        reason,
        order_id,
      },
    ...
    if(res.result==true){
      ...
      await showToast({title:"服务器出了点问题"})
    }
  ...
  async upLoadImg(report_id) {
    for (let imagePath of this.data.chooseImgs) {
      const res = await uploadFile({
        url: "/upload_report_img",
        filePath: imagePath,
        name: "image",
